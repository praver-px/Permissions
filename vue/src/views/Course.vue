<template>
  <div>
    ...
    <div style="margin: 10px 0">
      <el-button type="primary" @click="handleAdd" v-if="user.role !== 'ROLE_STUDENT'">新增 <i
      ...
      <el-popconfirm
          ...
          @confirm="delBatch"
      >
        <el-button type="danger" v-if="user.role !== 'ROLE_STUDENT'" slot="reference">批量删除 <i class="el-icon-remove-outline"></i></el-button>
      </el-popconfirm>
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"
      ...
      <el-table-column label="操作" width="280" align="center">
        <template slot-scope="scope">
          ...
          <el-button type="success" @click="handleEdit(scope.row)" v-if="user.role !== 'ROLE_STUDENT'">编辑 <i
          ...
          >
            <el-button type="danger" slot="reference" v-if="user.role !== 'ROLE_STUDENT'">删除 <i
...
export default {
  ...
  methods: {
    selectCourse(courseId) {
      this.request.post('/course/checkCourse/' + courseId + "/" + this.user.id).then(res => {
        if (res.code === '200') {
    ...
    load() {
      this.request.get("/course/page", {params: this.params}).then(res => {
        if (res.code === '200') {
